...
class JoyconRobotics:
    def __init__(self, 
        ...
        if device == "right":
            ...
            raise ValueError(f"Invalid device name: '{device}'. Must be 'right' or 'left'.")

        if self.joycon_id[0] is None:
            raise RuntimeError(
                f"No {device} Joy-Con found. Please ensure:\n"
                f"  1. The {device} Joy-Con is paired via Bluetooth\n"
                f"  2. The hid-nintendo driver is installed (run 'make install' first)\n"
                f"  3. The Joy-Con is powered on"
            )
